...
let unorderedCarYears = [];
/* let carYears; */ // See comment below


for (let i = 0; i < inventory.length; i++) {
    let carYear = inventory[i].car_year;
    unorderedCarYears.push(carYear);
    unorderedCarYears.sort();
    //Remove duplicates, conflict with challenge 5
    // carYears = [...new Set(unorderedCarYears)];

}

console.log(unorderedCarYears);
...
for (let i = 0; i < unorderedCarYears.length; i++) {
    if (unorderedCarYears[i] < 2000) {
        oldCars.push(unorderedCarYears[i]);
    } else {
        break;
    }
